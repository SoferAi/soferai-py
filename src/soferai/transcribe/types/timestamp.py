# This file was auto-generated by Fern from our API Definition.

import typing

import pydantic

from ...core.pydantic_utilities import UniversalBaseModel
from .language import Language


class Timestamp(UniversalBaseModel):
    word: str = pydantic.Field()
    """
    Word
    """

    punctuated_word: str | None = pydantic.Field(default=None)
    """
    Punctuated word
    """

    languages: list[Language] | None = pydantic.Field(default=None)
    """
    Language for the transcription that this timestamp belongs to
    """

    start: float = pydantic.Field()
    """
    Start time of the timestamp
    """

    end: float = pydantic.Field()
    """
    End time of the timestamp
    """

    speaker: str | None = pydantic.Field(default=None)
    """
    Speaker (Pro feature)
    """

    model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
